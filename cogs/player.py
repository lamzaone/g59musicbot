...
class Player(commands.Cog):
    ...
    def is_owner_or_has_dj_role(ctx):
    ...
    async def play(self, ctx, *, query: str = None):
            ...
            try:
                if "list=" in query:
                    ...
                    if "watch?v=" in query or 'youtu.be' in query or "youtube.com" in query:
                        message = await ctx.send("This is url/link of a playlist. Do you want to add the entire playlist to the queue?")
        ...
        try:
            ...
            if len(queue) > 0 or ctx.bot.repeat != "no":
                try:
                    # if ctx.bot.random == "on":
                    #     next_song = Queues.next_random_song(ctx.guild.id, ctx.bot.video_info['original_url'])
    ...
    ### RANDOM COMMAND ###
    # @commands.command(name='random', help='on/off random mode that playing random song in queue(auto shuffle)')
    # @commands.guild_only()
    # async def random(self, ctx):
    #     if ctx.voice_client and ctx.voice_client.is_playing():
    #         if ctx.bot.random == "on":
    #             ctx.bot.random = "off"
    #             await ctx.send(":game_die: Stoped playing random songs in queue.")
    #         else:
    #             ctx.bot.random = "on"
    #             await ctx.send(":game_die: Started playing random songs in queue.")
    ...
    @commands.guild_only()
    @commands.check(is_owner_or_has_dj_role)
